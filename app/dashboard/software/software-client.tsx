...
export function SoftwareClient({ categories }: { categories: CategoryData[] }) {
    const { t } = useLanguage()

    return (
        <div className="space-y-6">
            <div>
                <h2 className="text-2xl sm:text-3xl font-bold mb-2">{t('softwarePage.title')}</h2>
                <p className="text-muted-foreground">
                    {t('softwarePage.subtitle')}
                </p>
            </div>
                ...
                <div key={category.id}>
                    ...
                    {category.software.length === 0 && (
                        <Card className="p-8 text-center text-muted-foreground">
                            {t('softwarePage.noSoftwareInCategory')}
                        </Card>
                    )}
                </div>
            ))}
            ...
            {categories.length === 0 && (
                <Card className="p-12 text-center">
                    ...
                    <p className="text-muted-foreground">{t('softwarePage.noSoftware')}</p>
                </Card>
            )}
        </div>
    )
...
function SoftwareCard({ software }: { software: SoftwareData }) {
    const { t } = useLanguage()
